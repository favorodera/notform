---
title: Quickstart
description: Quick walk through of how to use notform.
navigation:
  icon: i-lucide-play
---
...
::card-group
  :::app-card{icon="lucide:text-cursor-input" title="NotField" to="/components/not-field"}
  All slot props, per-field trigger overrides, and singleton usage.
  :::

  :::app-card{icon="lucide:list" title="NotArrayField" to="/components/not-array-field"}
  Dynamic lists with append, remove, swap, and more.
  :::

  :::app-card{icon="i-lucide-message-circle-warning" title="NotMessage" to="/components/not-message"}
  Display error messages for fields.
  :::

  :::app-card{icon="lucide:code" title="useNotForm" to="/composables/use-not-form"}
