...
```ts
// v1
useNotForm({ initialState: { email: '' }, schema })

// v2
useNotForm({ initialValues: { email: '' }, schema })
```
...
```ts
// v1
// input is implicitly off
useNotForm({ schema, validateOn: [
  'blur',
  'change',
] })
...
useNotForm({ schema, validateOn: { onInput: false } })
```
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <input
          v-model="form.values.hobbies[item.index]"
          v-bind="events"
        >
      </NotField>
...
```vue
<template>
  <button @click="swap(0, 2)">
    Swap first and third
  </button>

  <button @click="move(3, 0)">
