...
#code
  :::code-collapse
    ::docs-demo-code{file="validation-modes"}
    ::
  :::
::
...
if (result.issues) {
  ...
  // Typed, transformed schema output
...
#code
  :::code-collapse
    ::docs-demo-code{file="server-errors"}
    ::
  :::
::
...
```ts
form.setErrors([
  { message: 'Already taken', path: [{ key: 'email' }] },
  { message: 'Too short', path: [{ key: 'username' }] },
])
```
