...
NotForm serves as the logic engine for your forms, handling state, validation, and submission, while shadcn-vue provides the visual building blocks. By using the `NotField`{lang="ts-type"} component, you can "wrap" any shadcn control to inject validation logic without sacrificing styling freedom.
...
Validation is handled automatically whenever state changes or a field loses focus. NotForm's deep integration with Zod means you only need to define your rules once in the schema.
...
Communicating errors effectively is key to a good user experience. By passing `!!errors.length`{lang="ts-type"}  to the `:data-invalid`{lang="ts-type"}  prop, shadcn components can automatically switch to their ["error"]{class="text-error"} styling (e.g., red borders).
...
Use the `NotArrayField` component to manage array fields. It provides `fields`{lang="ts-type"}, `append`{lang="ts-type"},`remove`{lang="ts-type"} and [other methods](/components/not-array-field#slots) through its slot props.
