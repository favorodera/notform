...
import type { ArrayFieldContext, ArrayFieldProps, ArrayFieldSlots } from '../types/array-field'
 ...
 */
const props = defineProps<ArrayFieldProps<TArraySchema>>()

/**
 * Slots provided by the ArrayField component.
 */
defineSlots<ArrayFieldSlots<TArraySchema>>()

// Retrieve the ID of the form this array field belongs to
const formID = inject(CURRENT_FORM_ID_KEY)

if (!formID) {
  throw new Error('ArrayField must be used inside a Form component')
}
...
const context = reactive({
  ...
  insert,
  update,
}) as ArrayFieldContext<TArraySchema>
</script>
